...
const electronAPI = {
  ...
  app: {
    ...
    downloadSkill: (url, skillName) => ipcRenderer.invoke("app:downloadSkill", url, skillName),
  ...
  session: {
    ...
    search: (keyword, limit) => ipcRenderer.invoke("session:search", keyword, limit)
  },
  // 调试工具
  debug: {
  ...
  // 安全校验（主进程侧邀请码状态守卫）
  security: {
    verifyInviteCode: (userId, inviteCode) => ipcRenderer.invoke("security:verifyInviteCode", userId, inviteCode),
    isInviteVerified: (userId) => ipcRenderer.invoke("security:isInviteVerified", userId),
    clearInviteStatus: (userId) => ipcRenderer.invoke("security:clearInviteStatus", userId)
  },
  // 安全存储（基于 OS 级加密的 safeStorage）
  safeStorage: {
    encrypt: (plaintext) => ipcRenderer.invoke("safeStorage:encrypt", plaintext),
    decrypt: (encrypted) => ipcRenderer.invoke("safeStorage:decrypt", encrypted),
